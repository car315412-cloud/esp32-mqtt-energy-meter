...
static void drawTextScreen(const char *line1, const char *line2, const char *line3, const char *line4, const char *line5)
{
  display.clearDisplay();
...
static void drawStatus()
{
  ...
  if (lastReadOk) {
    ...
    if (lastEnvOk) {
      snprintf(line4, sizeof(line4), "T=%.0f H=%.0f", lastTemp, lastHumi);
    } else {
      snprintf(line4, sizeof(line4), "T=-!!! H=-!!!");
    }
  } else {
  ...
  if (lastLight >= 0) {
    snprintf(line5, sizeof(line5), "L=%d R=%s S=%d", lastLight, relayOn ? "ON" : "OFF", sg90Angle);
  } else {
    snprintf(line5, sizeof(line5), "L=-!!! R=%s S=%d", relayOn ? "ON" : "OFF", sg90Angle);
  }

  drawTextScreen(line1, line2, line3, line4, line5);
}
