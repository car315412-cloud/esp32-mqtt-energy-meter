...
static void drawStatus()
{
  ...
  if (lastReadOk) {
    ...
    snprintf(line3, sizeof(line3), "Hz=%.1f PF=%.0f%% R=%s", lastFrequency, lastPf * 100.0f, relayOn ? "ON" : "OFF");
  } else {
    snprintf(line1, sizeof(line1), "V=-!!! I=-!!!");
    snprintf(line2, sizeof(line2), "W=-!!! kWh=-!!!");
    snprintf(line3, sizeof(line3), "Hz=-!!! PF=-!!! R=%s", relayOn ? "ON" : "OFF");
  }

  drawTextScreen("17 ENERGY MQTT", line1, line2, line3);
}
...
static void handleRelayCommand(const char *payload, size_t length)
{
  ...
  JsonVariant relayValue = doc["RELAY"];
  if (relayValue.isNull()) {
    return;
  }

  const char *state = relayValue.as<const char *>();
  if (state == nullptr) {
    return;
  }
...
static void mqttCallback(char *topic, byte *payload, unsigned int length)
{
  ...
  handleRelayCommand(message, copyLength);
}
